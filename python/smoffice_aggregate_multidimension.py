...
if reduced_order_model.set_analysis(ANALYSIS_ID):
    print "reduced_order_model loaded"
else:
    print "Could not load the reduced_order_model"
    exit(1)


# load in all the models
single_result = reduced_order_model.yhat('HeatingElectricity', 1, 1, 1, 1, 1, 1)
results = reduced_order_model.yhat('HeatingElectricity', data)


for covariate in reduced_order_model.covariate_names:
    print covariate

# verify that the covariates are all defined in the sweep file
analysis = AnalysisDefinition('sweep.json')
